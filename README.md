...
This is a library which implements the `Twitch::Connection` interface needed
by the `Twitch::Messaging` class, in terms of the network connection
abstraction provided by the SystemAbstractions library.

## Usage

The `TwitchNetworkTransport::Connection` class is basically
an adapter to implement `Twitch::Connection` using an underlying
`SystemAbstractions::NetworkConnection` object.
